...
- [1. Articulación Vertical–Horizontal en la Fase 4](#1-articulacion-verticalhorizontal-en-la-fase-4)
- [2. Coherencia de Políticas e Identidad (Sistema 5)](#2-coherencia-de-politicas-e-identidad-sistema-5)
  - [2.1 Cadena de Transmisión de Identidad (Vertical Descendente)](#21-cadena-de-transmision-de-identidad-vertical-descendente)
  - [2.2 Alineación Ascendente (Vertical hacia arriba)](#22-alineacion-ascendente-vertical-hacia-arriba)
- [3. Coherencia de Estrategias (Sistema 4)](#3-coherencia-de-estrategias-sistema-4)
  - [3.1 Verificación de Compatibilidad entre S4 de Niveles](#31-verificacion-de-compatibilidad-entre-s4-de-niveles)
- [4. El Canal Algedónico (Algedonic Loop) — Sistema de Alarma de Viabilidad](#4-el-canal-algedonico-algedonic-loop-sistema-de-alarma-de-viabilidad)
  - [4.1 Variables Críticas, Sensores y Umbrales](#41-variables-criticas-sensores-y-umbrales)
  - [4.2 Protocolos de Emergencia Pre-diseñados](#42-protocolos-de-emergencia-pre-disenados)
- [5. Los 6 Canales de Control Cibernético Vertical](#5-los-6-canales-de-control-cibernetico-vertical)
  - [5.1 Análisis del Canal 4 (C4): Los 8 Componentes de Transducción](#51-analisis-del-canal-4-c4-los-8-componentes-de-transduccion)
- [6. Niveles de SLA (Service Level Agreements): Metodología y Estructura (C1 y C4)](#6-niveles-de-sla-service-level-agreements-metodologia-y-estructura-c1-y-c4)
  - [6.1 SLA Externos: Metodología de Determinación en 5 Pasos](#61-sla-externos-metodologia-de-determinacion-en-5-pasos)
  - [6.2 SLAs Internos (Mecanismos S2 Corporativos)](#62-slas-internos-mecanismos-s2-corporativos)
...
| **Nivel 1.2 (B2C)** | Head of Growth (S5 local) | *"Nunca vender como característica algo que no exista en producción. No usar dark patterns en el funnel de conversión."* |
| **Nivel 1.3 (B2B)** | Head of Sales (S5 local) | *"Los contratos institucionales no pueden incluir cláusulas de cesión de datos de estudiantes a la universidad sin consentimiento individual."* |
| **Nivel 1.4 (Soporte/Infra)** | Head of CS + DevOps Lead (S5 local) | *"En ninguna circunstancia se puede acceder al contenido privado de los mazos de un usuario para diagnosticar un bug sin su autorización explícita."* |
...
| **S4 Corporativo ↔ S4 Unidades** | La dirección estratégica decide expandirse a un nuevo segmento de mercado (ej. corporativo B2B Enterprise) en Q2, pero Soporte/Infra no tiene capacidad para dar SLA a ese segmento hasta Q4. | El S4 corporativo *consulta* los modelos de capacidad del S4 de Infra antes de comprometer fechas de expansión. Los outputs del S4 de Infra son *inputs* del plan corporativo (principio de modelos anidados del Cap. 2). |
...
| **Institucional** | Estado del piloto con docentes | Seguimiento automatizado en CRM/GitHub Projects | Docente sin responder por 2 semanas | Docente indica insatisfacción o duda | Pérdida del único piloto activo en el semestre | **2 semanas sin respuesta:** Riesgo de enfriamiento del piloto en semestres cortos. **Pérdida de piloto:** Pérdida absoluta de validación institucional. Medido por Relaciones Institucionales. |
...
*   **Cierre del Bucle Algedónico con el Sistema 5:** Todos los protocolos de emergencia algedónicos expuestos culminan de manera obligatoria con un paso final donde la **Junta de Fundadores (Sistema 5)** sesiona de manera extraordinaria para evaluar el nivel de daño post-crisis y tomar la decisión definitiva sobre la viabilidad de la empresa, deliberando sobre si la situación requiere un giro radical (pivot), una reestructuración de la identidad organizacional o el cese ordenado de las operaciones de YachaqAI.
...
| :--- | :--- | :--- | :--- | :--- |
| **C1** | Absorción del entorno | Canal vertical de conexión y absorción natural de variedad entre los entornos de las distintas unidades operativas (ej. la interacción natural e informal que ocurre afuera entre los estudiantes universitarios del B2C y los docentes del B2B, la cual absorbe complejidad sin que Synapta deba intervenir). | Entornos de las unidades operativas | Metasistema (S3/S4/S5) |
...
El Cap. 2 exige que cada canal sea estructurado con 8 componentes de comunicación simétricos para evitar distorsión del mensaje. Se detalla el Canal 4 (Negociación de Recursos) como ejemplo completo:

1. **Emisor:** CTO (Líder de Ingeniería S1.1).
2. **Transductor 1 (Codificación de Salida):** GitHub Projects traduce entregables de software a indicadores cuantitativos (velocidad en puntos de historia, incidentes de API, deuda técnica en horas).
3. **Canal 1 (Vía de Ida):** Reunión semanal de Sprint Review + cuadro de mando semanal de la unidad en Google Sheets.
4. **Transductor 2 (Decodificación de Entrada):** El CFO/CEO evalúa las métricas y las decodifica en un reporte ejecutivo simplificado: *"Se completaron 48/50 puntos. Se necesita un ajuste de APIs de S/. 150/mes"*.
5. **Receptor:** CEO / CFO (S3 corporativo).
6. **Transductor 3 (Codificación de Retorno):** El receptor (CFO/CEO) aprueba el ajuste y actualiza el presupuesto mensual aprobado en el Google Sheet central de finanzas.
7. **Canal 2 (Vía de Retorno):** Conexión HTTPS del webhook de actualización de Google Sheets hacia Slack/GitHub.
8. **Transductor 4 (Decodificación de Retorno):** Consola de GitHub Actions que recibe el webhook y actualiza automáticamente la planificación de sprints en el tablero del CTO (Emisor original), informándole que el presupuesto ha sido recalibrado y el ciclo de negociación se ha cerrado con éxito.

- **Capacidad y Cadencia:** Capacidad máxima de 4 negociaciones simultáneas. Cadencia mensual o semanal ante desviaciones operativas.
...
1. **Paso 1 — Medición de Baseline Real:** Antes de comprometer niveles de servicio, el equipo opera YachaqAI en fase cerrada durante 2 semanas. El SLA nunca superará el percentil 80 (P80) observado.
   - *Justificación del P80:* Al ser un equipo con dedicación mixta/parcial, un P90 o P95 sería insostenible durante periodos de alta demanda por compromisos profesionales concurrentes o hitos de actividades paralelas. El P80 representa el punto de equilibrio de nuestra capacidad actual [4].
...
| :--- | :--- | :--- | :--- | :--- |
| **Usuario Beta** (Entorno inmediato, 10-30 alumnos) | Soporte directo por WhatsApp; respuesta en ≤ 12 horas en días hábiles. Acceso a nuevas funciones. | Log de tickets en GitHub Projects (automatizado por integraciones). | Disculpa directa y registro de la causa raíz en el cuadro de mando de Growth. Sin penalidades. | El círculo de confianza ofrece pruebas a cambio de soporte ágil y cercano. 12 horas es viable bajo la dedicación actual del equipo [5]. |
| **Usuario B2C** (Masivo digital, 50-150 usuarios) | Servicio disponible en horario operativo habitual (lunes a sábado, 7am–10pm). Soporte por correo o formulario; respuesta en ≤ 48 horas. | Reportes automáticos de UptimeRobot y logs de tickets del formulario. | Comunicación pública en el canal de difusión con explicación y tiempo estimado de resolución. | **Horario operativo:** Evita soporte nocturno insostenible para el equipo de dedicación parcial. **48h:** Límite para evitar frustración de uso [5]. |
| **Docente del Piloto** (Stakeholder institucional B2B) | Servicio disponible durante horas de clase (ej. L-M 10am–12pm). Soporte directo al WhatsApp del CEO; respuesta en ≤ 4 horas en días hábiles. Cambios en producción notificados con ≥ 48h. | Registro manual de incidencias durante horas de clase y timestamps de respuesta. | CEO contacta al docente en < 2h para explicar la causa y ofrece compensación (apoyo en personalización de contenido o extensión del piloto). | **4h:** Plazo crítico para evitar que el docente aborte el piloto si falla durante una clase [5]. La compensación simbólica preserva el prestigio institucional de Synapta. |
...
| **Relaciones Institucionales** | Ingeniería | No prometer características fuera de producción que no estén como "listas" en GitHub Projects/Issues. | Se discute en la SAS; Ingeniería asume control directo del roadmap frente al cliente. | Evita la sobrecarga por "deuda de expectativas" comerciales. |
...
| **Cualquier Integrante** | Todos | Responder a consultas directas en el canal de WhatsApp del equipo en ≤ 4 horas. | Se aborda en la SAS para evaluar sobrecarga de tareas y redistribuir carga. | Tiempo máximo para no paralizar la toma de decisiones colectiva [6]. |

*Protocolo ante Incumplimiento Reiterado:* Se define como incumplir el mismo SLA interno **2 o más veces en el mismo mes**. Gatilla revisión obligatoria en el Bloque 1 de la SAS mensual para diagnosticar si se debe a sobrecarga de actividades externas, falta de herramientas, o si se requiere redefinir el compromiso.
...
| [2] | Pérez Ríos, José (2012). *Diseño y Diagnóstico para Organizaciones Sostenibles*. Editorial Ibergarceta. | Teoría de los 10 componentes del canal de comunicación, definición del canal algedónico, patologías de sistemas de información, y principios del metasistema en el Modelo de Sistema Viable |
| [3] | Beer, Stafford (1979). *The Heart of Enterprise*. John Wiley & Sons. | Fundamento de los canales cibernéticos verticales y horizontales, balance de variedad y loops algedónicos organizacionales |
| [4] | Beyer, B., Jones, C., Petoff, J., & Murphy, K. (2016). *Site Reliability Engineering: How Google Runs Production Systems*. O'Reilly Media. | Uso del percentil 80 (P80) y percentiles intermedios para el diseño y calibración inicial de Service Level Objectives (SLOs) y SLAs en entornos con recursos limitados |
| [5] | Zendesk (2024). *Customer Experience Trends Report 2024* | Estándares de la industria para tiempos de respuesta a soporte al cliente (≤ 48h para email, ≤ 12h para mensajería digital y ≤ 4h para soporte telefónico o preferente) |
| [6] | GitLab (2022). *Remote Work Report* | Las mejores prácticas de comunicación asíncrona establecen un tiempo máximo de respuesta de 4 horas para mantener la agilidad del equipo sin interrumpir el enfoque profundo |